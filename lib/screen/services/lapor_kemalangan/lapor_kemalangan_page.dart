...
class LaporKemalanganPage extends StatelessWidget {
  const LaporKemalanganPage({Key? key}) : super(key: key);
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  if(state.lat != "")
                    const _LatLong(),
                  if(state.pilihMethodAlamat == "2")
                    const _ButtonLocation(),
                ],
              ),
              if(state.pilihMethodAlamat =="1")
                const _AlamatBencana(),
...
class _PilihBencana extends StatelessWidget {
  const _PilihBencana({Key? key}) : super(key: key);
    ...
    return Consumer<LaporKemalanganState>(
      builder: (_, state, __) {
        return AppSelectField(
          label: 'Bencana',
          hint: 'Sila Pilih...',
          value: state.bencanaType,
          items: _mapItems(),
          onChange: (v) => state.bencanaType = v,
        );
      },
    );
  }

  List<DropdownMenuItem<dynamic>> _mapItems() {
    return [
      'Ribut',
      'Banjir',
      'Lain-Lain',
    ].map((e) => appDropDownMenuItem(e, e)).toList();
  }
}

class _LainLain extends StatelessWidget {
  const _LainLain({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Consumer<LaporKemalanganState>(
      builder: (_, state, __) {
        return AppTextField(
          label: 'Lain-Lain Bencana',
          maxLine: 3,
          onChange: (v) => state.others = v,
          onSubmitted: (v) => state.others = v,
          errorText: state.othersHasError ? state.othersError : null,
        );
      },
    );
  }
          ...
          child: Container(
            ...
            child: InkWell(
              ...
              child: const Padding(
                ...
                child: TextManrope(text: "Dapatkan Lokasi",
